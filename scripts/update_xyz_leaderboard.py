...
from src.utils.utils import get_xyz_token_mappings
...
def update_leaderboard_data():
    ...
    params = {
        "coin_list": ','.join([f"'{coin}'" for coin in xyz_token_names])
    }
    ...
    leaderboard_query_id = os.getenv("ALLIUM_XYZ_LEADERBOARD_QUERY_ID")
    if not leaderboard_query_id:
        logger.error("ALLIUM_XYZ_LEADERBOARD_QUERY_ID environment variable not set")
        return False
